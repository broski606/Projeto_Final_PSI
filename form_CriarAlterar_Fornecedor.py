...
from Interfaces.formCriarAlterarFornecedor import Ui_MainWindow
...
class formCriarAlterarFornecedor(QtWidgets.QMainWindow, Ui_MainWindow):
    def __init__(self, formFornecedores):
        super().__init__()
        self.setupUi(self)
        # Definição vars
    ...
    def gravar(self):
        ...
        if self.modo_funcionamento == "novo":
            try:
                ...
                cmd_sql = f"SELECT COUNT(*) FROM Fornecedor WHERE id = %s OR nif = %s;"
                numRegistos = consultaUmValor(conn_BD, cmd_sql, (id, nif,))
                if numRegistos == -1:
                    QtWidgets.QMessageBox.critical(self, "Erro", "Ocorreu um erro ao verificar a existência do fornecedor")
                    return
                elif numRegistos > 0:
                    QtWidgets.QMessageBox.critical(self, "Aviso", f"Já existe um fornecedor com nif {nif} ou com id {id} introduzidos")
                    return

                cmd_sql = "INSERT INTO Fornecedor (id, nome, nif, morada, telefone, email, ativo) VALUES (%s, %s, %s, %s, %s, %s, %s);"
                numRegistos = operacao_DML(conn_BD, cmd_sql, (id, nome, nif, morada, telefone, email, ativo))
                if numRegistos == -1:
                ...
                resposta = QtWidgets.QMessageBox.question(
                    self,
                    "Confirmação",
                    "Fornecedor inserido com sucesso!\n Pretende inserir dados de um novo fornecedor?",
            ...
            try:
                ...
                if len(nome) == 0:
                    QtWidgets.QMessageBox.critical(self, "Aviso", "Nome do fornecedor por preencher")
                    return
                ...
                # Verificar se o fornecedor existe (por id)
                cmd_sql = "SELECT COUNT(*) FROM Fornecedor WHERE id = %s;"
                numRegistosId = consultaUmValor(conn_BD, cmd_sql, (id,))

                # Verificar se já existe outro fornecedor com o mesmo NIF (exclui o próprio fornecedor que está a ser alterado)
                cmd_sql = "SELECT COUNT(*) FROM Fornecedor WHERE nif = %s AND id <> %s;"
                ...
                if numRegistosId == -1 or numRegistosNif == -1:
                    ...
                    QtWidgets.QMessageBox.critical(self, "Aviso", f"Não foi possível encontrar o fornecedor com identificador {id}!")
                    return
                elif numRegistosNif > 0:
                    QtWidgets.QMessageBox.critical(self, "Aviso", f"Já existe outro fornecedor com o nif {nif} introduzido")
                    return

                cmd_sql = "UPDATE Fornecedor SET nome = %s, nif = %s, morada = %s, telefone = %s, email = %s, ativo = %s WHERE id = %s;"
                numRegistos = operacao_DML(conn_BD, cmd_sql, (nome, nif, morada, telefone, email, ativo, id,))
                if numRegistos == -1:
                    QtWidgets.QMessageBox.critical(self, "Erro", "Ocorreu um erro ao alterar o registo")
                    return

                QtWidgets.QMessageBox.information(self, "Confirmação", "Fornecedor alterado com sucesso!")
    ...
    def inicializar(self, selecao, modo_funcionamento):
        ...
        if modo_funcionamento == "novo":
            ...
            try:
                ...
                cmd_sql = "SELECT MAX(id)+1 FROM Fornecedor;"
            ...
            modelo = self.form_Fornecedores.tableView.model()
            ...
            self.lineEdit_Telefone.setText(modelo.data(modelo.index(linha, 4)))
            self.lineEdit_Email.setText(modelo.data(modelo.index(linha, 5)))
            self.checkBox.setChecked(True)
